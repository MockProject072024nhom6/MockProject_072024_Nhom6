...
const ContractTable: React.FC<I_props_ContractTable> = ({ contracts }) => {
  ...
  const filteredContracts = contracts.filter(contract => {
    return (
      contract.typeOfService.toLowerCase().includes(searchTypeOfService.toLowerCase()) &&
      contract.status.toLowerCase().includes(searchStatus.toLowerCase()) &&
      contract.startDate.includes(searchStartDate) &&
      contract.contractCode.toLowerCase().includes(searchContractCode.toLowerCase())
    )
  })
  ...
  return (
    <div className='overflow-x-auto text-black mx-20'>
      ...
      <div className='flex mb-4 m-1'>
        ...
        />
        <button className='ml-10 bg-blue-500 text-white rounder-lg text-sm'>Add service</button>
      </div>
        ...
        <tbody>
          {currentContracts.map((contract, index) => (
            <tr key={index}>
              ...
              <td className='px-4 py-2 border-b'>{contract.specialRequirements}</td>
              ...
              <td className='px-4 py-2 border-b text-blue-500 cursor-pointer hover:underline'>{contract.view}</td>
      ...
      <div className='flex justify-between mt-4 items-center'>
        ...
        <span>
          Showing {indexOfFirstItem + 1} to {Math.min(indexOfLastItem, filteredContracts.length)} of{" "}
