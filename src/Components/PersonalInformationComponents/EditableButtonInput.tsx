...
const EditableButtonInput: React.FC<I_props_EditableButton> = ({ initialText, type, label }) => {
    ...
    <section>
      <div className='relative p-2 border-2 my-1 rounded-lg flex items-center w-9/12'>
        ...
        <div className='flex  items-center flex-grow justify-end '>
          ...
          {type === "password" && (
            <button onClick={toggleShowPassword} className='px-3 py-1 mr-2 rounded-lg bg-gray-300 hover:bg-gray-400'>
              {showPassword ? "Hide" : "Show"}
            </button>
          )}

          <button onClick={handleEditClick} className='px-3 mr-2 py-2  rounded-lg bg-gray-300 hover:bg-gray-400'>
            Edit
          </button>
        </div>
      </div>
